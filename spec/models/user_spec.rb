...
RSpec.describe User, type: :model do
  ...
  describe "is not saved" do
    it "with too short password" do
      u = User.create username: "Pekka", password: "S1", password_confirmation: "S1"

      expect(u).not_to be_valid
      expect(User.count).to eq(0)
    end

    it "with password only using lowercase" do
      u = User.create username: "Pekka", password: "abcd", password_confirmation: "abcd"
  ...
  describe "favorite style" do
    ...
    it "is the one with highest rating if several rated" do
      create_beers_with_many_ratings({user: user}, 10, 20, 15, 7, 9)
      beer = FactoryBot.create(:beer, style: "beststyle")
      best = FactoryBot.create(:rating, beer: beer, score: 25, user: user )

      expect(user.favorite_style).to eq("beststyle")
    end
  end
  ...
  describe "favorite brewery" do
    ...
    it "is the one with highest rating if several rated" do
      create_beers_with_many_ratings({user: user}, 10, 20, 15, 7, 9)
      br = FactoryBot.create(:brewery, name: "bestbrewery")
      beer = FactoryBot.create(:beer, brewery: br)
      best = FactoryBot.create(:rating, beer: beer, score: 25, user: user )
